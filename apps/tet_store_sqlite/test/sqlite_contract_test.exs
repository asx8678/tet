defmodule Tet.Store.SQLiteContractTest do
  ...
  describe "create_artifact/1 and list_artifacts/2" do
    test "creates an artifact", %{opts: _opts} do
      attrs = %{
        command: ["echo", "hello"],
        risk: :low,
        exit_code: 0,
  ...
  describe "append_step/2 and get_steps/2" do
    test "appends a step", %{opts: opts} do
      attrs = %{
        id: "step_001",
        workflow_id: "wf_001",
  ...
  describe "enqueue_repair/2, dequeue_repair/1, update_repair/3, list_repairs/1" do
    test "full repair lifecycle", %{opts: opts} do
      ...
      # List by session
      assert {:ok, session_repairs} =
               @adapter.list_repairs(session_id: "ses_repairs", repairs_path: opts[:repairs_path])

      assert length(session_repairs) >= 1

      # List by status
      assert {:ok, completed_repairs} =
               @adapter.list_repairs(status: :completed, repairs_path: opts[:repairs_path])
