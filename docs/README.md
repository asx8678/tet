...
`BD-0011`, compacted context from `tet-db6.12` / `BD-0012`, the editable
model registry schema from `tet-db6.13` / `BD-0013`, and read-only tool
contracts from `tet-db6.20` / `BD-0020`.
...
|---|---|---|
| `tet_core` | Pure domain/contracts boundary | Metadata, `%Tet.Event{}`, `%Tet.Message{}`, `%Tet.Session{}`, `%Tet.Autosave{}`, `Tet.Prompt` prompt-layer contract, `Tet.Compaction` compacted-context contract, model registry schema/validation, `Tet.Tool` read-only tool contracts, `Tet.Provider` behaviour, and `Tet.Store` behaviour |
