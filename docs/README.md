...
`BD-0011`, and the editable model registry schema from `tet-db6.13` /
`BD-0013`.
...
|---|---|---|
| `tet_core` | Pure domain/contracts boundary | Metadata, `%Tet.Event{}`, `%Tet.Message{}`, `%Tet.Session{}`, `%Tet.Autosave{}`, `Tet.Prompt` prompt-layer contract, model registry schema/validation, `Tet.Provider` behaviour, and `Tet.Store` behaviour |
