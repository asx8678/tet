...
`BD-0011`, compacted context from `tet-db6.12` / `BD-0012`, and read-only tool
...
|---|---|---|
| `tet_core` | Pure domain/contracts boundary | Metadata, `%Tet.Event{}`, `%Tet.Message{}`, `%Tet.Session{}`, `%Tet.Autosave{}`, `Tet.Prompt` prompt-layer contract, `Tet.Compaction` compacted-context contract, `Tet.Tool` read-only tool contracts, `Tet.Provider` behaviour, and `Tet.Store` behaviour |
